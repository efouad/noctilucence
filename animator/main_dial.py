...
#
# Creates a dial indicator and displays it on the screen. 
#

#TODO Implement for non-vertical directions
#TODO Better handle points behind the surface (e.g. capture surface outwardness)
def vert_ht_to_poly(origin, poly): 
    """ Returns the vertical distance to the top (+y) of Polygon surface.  
    
    Args:
        origin: Point to be measured from
        poly: Polygon to be measured to
    
    Returns:
        (float) Vertical distance from origin to the polygon surface, sharing
         the x-component and interpolating if needed between Polygon vertices.
    """
    
    # Find vertices on poly with closest x components to origin.
    closest_P_above = None
    closest_P_below = None
    for point in poly.attributes["components"]:
        if (point.gpos()[0] - origin.gpos()[0] >= 0 and
               (closest_P_above is None or 
                    point.gpos()[0] - origin.gpos()[0] < 
                        closest_P_above.gpos()[0] - origin.gpos()[0])):
            closest_P_above = point
        if (origin.gpos()[0] - point.gpos()[0] > 0 and
               (closest_P_below is None or
                    origin.gpos()[0] - point.gpos()[0] < 
                        origin.gpos()[0] - closest_P_below.gpos()[0])):
            closest_P_below = point
            
    # Interpolate Y-height between two closest vertices.
    y_interpol = (closest_P_below.gpos()[1] + 
            (closest_P_above.gpos()[1] - closest_P_below.gpos()[1]) * 
            (origin.gpos()[0] - closest_P_below.gpos()[0]) / 
            (closest_P_above.gpos()[0] - closest_P_below.gpos()[0]))
    
    return (origin.gpos()[1] - y_interpol)
